...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    bob = generate_points_on_circle(circle, number_of_points)
    for k in range(len(bob)):
        bob[k].attach_to(window)
        tim = rg.Circle(bob[k], 10)
        tim.fill_color = color
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  5 slices, thin (thickness=4) red lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 4
    pizza(window, circle, 5, 'red', 4)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    bob = generate_points_on_circle(circle, number_of_slices)
    for k in range(len(bob)):
        tim = rg.Line(bob[k], circle.center)
        tim.color = color
        tim.thickness = thickness
        tim.attach_to(window)
    window.render()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    bob = generate_points_on_circle(circle, number_of_segments)
    for k in range(len(bob) - 1):
        tim = rg.Line(bob[k], bob[k+1])
        tim.thickness = thickness
        tim.color = color
        tim.attach_to(window)
    jerry = rg.Line(bob[len(bob) - 1], bob[0])
    jerry.thickness = thickness
    jerry.color = color
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  20 red lines on blue circle, hops = 5.'
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    bob = generate_points_on_circle(circle, number_of_lines)
    for k in range(len(bob)):
        x = k + hops_to_next_point
        if x >= len(bob):
            x = x - len(bob)
        tim = rg.Line(bob[k], bob[x])
        tim.color = color
        tim.thickness = thickness
        tim.attach_to(window)
    window.render()
